...
static char * testHashInit() {
  ...
  muAssert("error, hash size shouldn't be NULL", hash.data != NULL);
  return 0;
}
static char * testHashInsert() {
  ...
  muAssert("error, first key cincuenta", strcmp(hash.data[0].elements[1].key,"cincuenta") == 0);
  muAssert("error, first key cuarenta", hash.data[0].elements[0].value == 40);
  muAssert("error, first key cincuenta", hash.data[0].elements[1].value == 50);
  return 0;
}
